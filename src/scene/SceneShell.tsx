...
import { CityDiagram } from "../gfx/CityDiagram";
import { AnimatedMap } from "../gfx/AnimatedMap";
import { Timeline } from "../gfx/Timeline";
...
const Built: React.FC<{ scene: Scene }> = ({ scene }) => {
  ...
  switch (scene.kind) {
    ...
    case "CITYDIAG": return <CityDiagram type={c.diagram} accent={scene.accent} />;
    case "MAP": return <AnimatedMap mode={c.mode} location={c.location} counterTo={c.counterTo} counterPrefix={c.counterPrefix} label={c.label} glow={scene.accent} />;
    case "TIMELINE": return <Timeline label={c.label} year={c.year} spanLabel={c.spanLabel} accent={scene.accent} />;
