...
import { SurvivingTwig } from "./motif/SurvivingTwig";
 ...
 *
 * Rules (enforced here, validated in build_data_sp.py): ease everything, settle by ~40% of the shot
 ...
 *
 * THE STYLE THIS VIDEO INVENTS:
 *   SCALE-REVEAL - "a slow pull-back that reveals how small the figures are against the space around
 *   them". It is reserved for the twelve shots whose whole point is scale or isolation, including the
 *   S001 / S180 / S218 callback of one figure alone in a wide landscape. So it is not a pull-back with
 *   a nice curve: the frame opens, the vignette OPENS with it rather than closing, and the figure is
 *   left smaller in more visible space at the end of the shot than at the start. That is the opposite
 *   gesture to CROWD-PRESS, which closes in and darkens the edges as it arrives, and to ISOLATE-DRIFT,
 *   which drifts outward while tightening.
 */
export const ShotScene: React.FC<{ shot: Shot }> = ({ shot }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: shot.stage === "DENIS" ? PAL.nearBlack : PAL.charcoal, overflow: "hidden" }}>
      <AbsoluteFill>
        {splitP === null ? (
          shot.depth ? (
            <>
      ...
      {shot.twig !== null ? <SurvivingTwig growth={shot.twig} seed={shot.i} /> : null}
