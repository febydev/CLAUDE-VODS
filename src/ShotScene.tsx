...
import { BogHaze } from "./fx/BogHaze";
import { RingAndOutside } from "./motif/RingAndOutside";
import { DashedBorder } from "./motif/DashedBorder";
import { StrikeOut } from "./motif/StrikeOut";
import type { Shot } from "./types";

/**
 * ANIMATION ENGINE — all 33 styles from FULL_PACKAGE section 10.
 *
 * Rules (enforced here, validated in build_data_pn.py): ease everything, settle by ~40% of the shot
 * then hold, each shot reverses the drift direction of the one before it, no two adjacent shots share
 * a style, nothing fully static for more than 1.0s. No CRAWL-style motion anywhere - the spec removes
 * it from this video deliberately.
 *
 * THE TWO PURPOSE-BUILT STYLES CARRY THE ARGUMENT, so they are not decorative camera moves:
 *
 *   ISOLATE-DRIFT (S009 S053 S064 S066 S069 S096 S169) - the frame drifts OUTWARD, so the figure
 *     ends the shot smaller and further away than it started, and the vignette closes in behind it.
 *     This is the exclusion beat: nobody touches him, the world simply gets bigger around him.
 *
 *   CROWD-PRESS (S001 S004 S028 S056 S068 S086 S157 S168 S176 S179 S205 S208) - the opposite: a slow
 *     inward creep with the edges darkening as they come, so the surround closes on the viewer. Used
 *     where the group IS the pressure, including the two crucifixion crowd shots that point at
 *     something outside the frame.
 *
 * Plus BRANCH-TRACE (branching cause diagrams trace outward from their root) and SCALE-TIP (balance
 * beams tip and settle rather than pan).
 */
export const ShotScene: React.FC<{ shot: Shot }> = ({ shot }) => {
  ...
  let flicker = 1;
  let vigBoost = 0; // ISOLATE-DRIFT / CROWD-PRESS close the frame down as they move
  ...
  switch (shot.anim) {
    // ── the two purpose-built styles (section 10) ──────────────────────────────────────────
    case "ISOLATE-DRIFT":
      // drift outward: he gets smaller, the empty space gets bigger, the edges close in
      ...
      break;
    case "CROWD-PRESS":
      // creep inward: the surround closes on the frame and darkens as it arrives
    ...
    case "STACK-BUILD":
      ...
      break;
    case "CROSS-OUT":
      // the negation beat: one band of doubt sweeps the frame and leaves. Deliberately NOT a drawn
      // cross - the hand-drawn annotation is already inside the image.
    ...
    case "TILT-PAN-DOWN":
      ...
      break;
    case "BRANCH-TRACE":
      // A branching cause diagram traced outward from its root, then held complete.
      // It gets its OWN ramp rather than the shared `settle`: EASE is bezier(0.22,1,0.36,1), which is
      // heavily front-loaded, so driving the reveal off settle finished the trace within the first
      // few frames of the shot - measured as a 0.7 luma difference between 12% and 95% through the
      // shot, i.e. invisible. A decelerating ramp across ~45% of the shot makes the trace readable
      // and still leaves more than half the beat holding complete.
      ...
      break;
    case "SCALE-TIP":
      // a balance beam: it tips one way, overshoots slightly, and settles level
  ...
  const g = GRADE[shot.grade] || GRADE.LIMITS;
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: shot.stage === "BOG" ? PAL.peatDark : PAL.charcoal, overflow: "hidden" }}>
      <AbsoluteFill>
        {splitP === null ? (
          shot.depth ? (
            <>
        ...
        {/* DIAGRAM-DRAW: reveal left to right on an ink edge, then hold complete */}
        {drawWipe !== null ? (
          <AbsoluteFill
            style={{
              pointerEvents: "none",
        ...
        {/* BRANCH-TRACE: the cover is a charcoal fill with a GROWING HOLE punched at the diagram's
            root, so the branches are revealed outward from where they start. An SVG mask is used
            rather than an inverted clip-path, because clip-path cannot express "everything except
            this circle" and the naive version reveals the wrong half of the frame. */}
        {branch !== null ? (
            ...
            <svg width={width} height={height} style={{ position: "absolute", inset: 0 }}>
              <defs>
                <mask id={`branchmask-${shot.i}`}>
                  ...
                  {/* 0.92 * width is just past the far corner from the root, so the reveal finishes
                      exactly as `branch` reaches 1 rather than long before it */}
                  <circle
      ...
      {shot.haze > 0.001 ? <BogHaze strength={shot.haze} seed={shot.i} tender={shot.tender} /> : null}

      {/* stage tint + stage vignette. ISOLATE-DRIFT and CROWD-PRESS tighten the vignette as they move. */}
      <AbsoluteFill style={{ background: g.tint, mixBlendMode: "soft-light", opacity: g.op }} />
      <AbsoluteFill
        style={{
          pointerEvents: "none",
          background: `radial-gradient(ellipse ${84 - vigBoost * 40}% ${84 - vigBoost * 40}% at 50% 48%, rgba(0,0,0,0) 48%, rgba(8,7,6,${
            g.vig + vigBoost
          }) 100%)`,
        }}
      />
      ...
      {shot.ring !== null ? (
        <RingAndOutside growth={shot.ring} seed={shot.i} cy={shot.y !== null && shot.y > 0.5 ? 0.44 : 0.54} />
      ) : null}
      {shot.dashed ? <DashedBorder seed={shot.i} /> : null}
      {shot.strike ? <StrikeOut pass={shot.strike} /> : null}
