...
export const Main: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: PAL.charcoal }}>
      {shots.map((shot, i) => (
        <Sequence key={i} from={shot.start} durationInFrames={shot.duration} name={`${shot.shot}-${shot.type}${shot.comedy ? "-JOKE" : ""}`} layout="none">
          <Entrance shot={shot}>
            <ShotScene shot={shot} />
          </Entrance>
          {/* tiktok SFX — contextual: caption landings, section pivots, stat reveals, the 7 comedy
              punches, finale accent. Modern-contrast shots are silent so the gag lands (§10). */}
          {shot.sfxFile ? (
      ...
      <Audio src={staticFile("VOICEOVER_ancient-jacked_FULL.mp3")} />
