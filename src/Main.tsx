...
export const Main: React.FC = () => {
  const shots = D.shots;
  return (
    <AbsoluteFill style={{ backgroundColor: PAL.night }}>
      {shots.map((shot, i) => (
        <Sequence key={i} from={shot.start} durationInFrames={shot.duration} name={`${shot.shot}-${shot.type}`} layout="none">
          <Entrance shot={shot}>
            <ShotScene shot={shot} />
          </Entrance>
          {/* tiktok SFX — contextual: caption landings, section pivots, insight reveals, finale.
              Darkness section left intentionally silent (§10). Narration stays dominant. */}
          {shot.sfxFile ? (
      ...
      <Audio src={staticFile("VOICEOVER_ancient-winter_FULL.mp3")} />
