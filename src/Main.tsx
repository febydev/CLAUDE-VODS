...
//
// v2 adds ONE exception: the thirteen arc pivots. Cutting 210 shots identically is a large part of
// why the v1 assembly felt like a slideshow - the viewer got no signal that the film had changed
// chapter. A pivot now clears on a 10-frame paper wipe. It is an overlay ON the incoming shot, so
// the in-point itself does not move by a single frame, and it is skipped where the shot's own style
// already carries a reveal gesture.
const FADE_UP = 7;
const FADE_OUT = 18;
const PIVOT = 10;
...
const Entrance: React.FC<{ shot: Shot; children: React.ReactNode }> = ({ shot, children }) => {
  ...
  // arc pivot: a torn-paper panel clears the frame, direction alternating with the drift rule
...
export const Main: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: PAL.charcoal }}>
      {D.shots.map((shot) => (
        ...
        >
          ...
          {/* SFX type 1 of 2: a soft reveal ding on structural and statistical caption landings,
              and on the three returns of the eye motif (the spec's recurring "opening bell tone").
              Synced to the LANDING frame of the animation, never fired on a 7A hedging beat. */}
          {shot.sfxFile ? (
      ...
      {/* ── single music bed, deliberately QUIET so the narrator is clearly the loudest thing in
             the mix (the user's note for this rebuild). Base sits at 0.072 versus v1's 0.115,
             which is about 4 dB lower, and the bed is EQ-carved around the voice when it is built.
             Still ducked across the CTA window and still saving the one largest swell for S210. ── */}
