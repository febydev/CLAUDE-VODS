...
// CTA end card (Section 9 Type 12). Bold takeaway, warm ember burst, glow border, energetic.
export const CTACard: React.FC<{ big: string; sub: string; accent: string }> = ({ big, sub, accent }) => {
  ...
  const subS = spring({ frame: frame - 34, fps, config: { damping: 200 } });
  ...
  return (
    <AbsoluteFill style={{ background: MG_BG, alignItems: "center", justifyContent: "center" }}>
      ...
      {new Array(40).fill(0).map((_, i) => {
        ...
        const dist = interpolate(frame, [0, 80], [0, 260 + random(`d${i}`) * 380], { extrapolateRight: "clamp" });
        const rise = frame * (0.3 + random(`v${i}`) * 0.9);
        const x = cx + Math.cos(ang) * dist, y = cy + Math.sin(ang) * dist * 0.5 - rise;
        const life = interpolate(frame, [0, 14, 90, 130], [0, 1, 1, 0], { extrapolateRight: "clamp" });
      ...
      <div style={{ position: "absolute", inset: 26, border: `2px solid ${accent}`, borderRadius: 14, opacity: 0.3 + pulse * 0.4, boxShadow: `inset 0 0 70px ${accent}44, 0 0 40px ${accent}33` }} />
      <div style={{ textAlign: "center", padding: "0 8%" }}>
        {big.split("\n").map((line, i) => (
          <div key={i} style={{ fontFamily: FONT, fontWeight: 900, fontSize: 96, color: "#FFF6EA", lineHeight: 1.06,
        ...
        <div style={{ marginTop: 40, fontFamily: FONT, fontWeight: 600, fontSize: 46, letterSpacing: 2, color: accent, opacity: subS, transform: `scale(${0.9 + subS * 0.1})` }}>{sub}</div>
      </div>
    </AbsoluteFill>
  );
};
