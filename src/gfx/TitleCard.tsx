...
export const TitleCard: React.FC<{ title: string[]; sub: string; accent: string }> = ({ title, sub, accent }) => {
  ...
  return (
    <AbsoluteFill style={{ background: "radial-gradient(ellipse at 50% 42%, #ffe3d6 0%, #f6c3b4 60%, #e9a899 100%)" }}>
      ...
      <AbsoluteFill style={{ alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          {title.map((line, i) => {
            ...
            return <div key={i} style={{ fontFamily: FONT, fontWeight: 800, fontSize: 132, lineHeight: 1.0, color: "#3a1626",
              opacity: interpolate(frame - (i * 7 + 4), [0, 6], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }),
              transform: `scale(${interpolate(s, [0, 1], [1.4, 1])})`, textShadow: "0 6px 20px rgba(120,40,60,0.25)" }}>{line}</div>;
          })}
          <div style={{ marginTop: 26, fontFamily: FONT, fontWeight: 600, fontSize: 44, color: "#7a3046", opacity: subO }}>{sub}</div>
        </div>
      </AbsoluteFill>
    </AbsoluteFill>
  );
